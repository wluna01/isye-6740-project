...
for media in dialogue['media'].unique():
    ...
    for speaker in top_speakers:
        #calculate the average length of each sentence spoken by the speaker
        ...
        #print(speaker_df['dialogue'].head())
        # Plotting the distribution as a KDE curve
        sns.kdeplot(sentence_lengths, bw_adjust=0.5)
        plt.xlabel('Sentence Length (words)')
        plt.ylabel('Density')
        #set the title to the speaker's name
        plt.title(speaker)
        #save the plot to a png file
        plt.savefig(f'../images/{speaker}_sentence_length.png')
